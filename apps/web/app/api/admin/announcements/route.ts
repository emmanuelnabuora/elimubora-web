...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await apiFetch('/v1/announcements', { method: 'POST', body: JSON.stringify(body) });
    return NextResponse.json(result, { status: 201 });
  } catch (err) {
    if (err instanceof ApiError) return NextResponse.json({ message: err.message }, { status: err.status });
    console.error('Announcements route: unexpected error', err);
