import { NextResponse } from 'next/server';
import { ApiError, apiFetch } from '../../../../lib/api-client';

/** Proxies POST /v1/lesson-plans (manual create) and, via ?ai=1, POST /v1/lesson-plans/ai-draft. */
export async function POST(request: Request): Promise<NextResponse> {
  const useAi = new URL(request.url).searchParams.get('ai') === '1';
  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ message: 'Malformed request body.' }, { status: 400 });
  }
  try {
    const plan = await apiFetch(useAi ? '/v1/lesson-plans/ai-draft' : '/v1/lesson-plans', {
      method: 'POST',
      body: JSON.stringify(body)
    });
    return NextResponse.json(plan, { status: 201 });
  } catch (err) {
    if (err instanceof ApiError) return NextResponse.json({ message: err.message }, { status: err.status });
    console.error('Lesson plans route: unexpected error', err);
    return NextResponse.json({ message: 'Something went wrong. Please try again.' }, { status: 502 });
  }
}
