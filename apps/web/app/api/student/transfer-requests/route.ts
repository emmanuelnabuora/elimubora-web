...
/** Proxies GET /v1/transfer-requests (student's own only) and POST (submit). */
export async function GET(): Promise<NextResponse> {
  try {
    ...
    console.error('Student transfer requests route: unexpected error', err);
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await apiFetch('/v1/transfer-requests', { method: 'POST', body: JSON.stringify(body) });
