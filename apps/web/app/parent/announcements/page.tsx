...
export default async function AnnouncementsPage() {
  const announcements = await apiFetch<Announcement[]>('/v1/parent-portal/announcements');
            ...
            <div key={a.id} className="admin-section" style={{ margin: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <h2 className="admin-section-title" style={{ marginBottom: 4 }}>
                  <Link href={`/parent/announcements/${a.id}`} style={{ color: 'inherit', textDecoration: 'none' }}>
                    {a.title}
                  </Link>
                </h2>
